...
if(!isset($datos->cedula)){
   ...
   // $password = doEncrypt($datos->password);
    ...
    if($conn==null){
       ...
       $stringSQL = 'UPDATE `profesor` SET `cedula`=:cedula,`correoelectronico`=:correoelectronico,`telefono`=:telefono,`telefonocelular`=:telefonocelular,`fechanacimiento`=:fechanacimiento,`sexo`=:sexo,`direccion`=:direccion,`nombre`=:nombre,`apellidopaterno`=:apellidopaterno,`apellidomaterno`=:apellidomaterno,`nacionalidad`=:nacionalidad,`usuario`=:usuario WHERE `id`= :id;';
       ...
       // $result = $conn->query($sql); //$result = true/false on success or error respectively.
        ...
        if ($result) {
            sendResponse(200, $result , 'Profesor Updating Successful.');
        } else {
            sendResponse(404, [] ,'Profesor not Updating');
        }
        //close connection
        $conn->close();
    }
