...
if(!isset($datos->cedula)){
   ...
   // $password = doEncrypt($datos->password);
    ...
    if($conn==null){
       ...
       $stringSQL = 'INSERT INTO `estudiante`(`id`, `cedula`, `correoelectronico`, 
       `telefono`, `telefonocelular`, `fechanacimiento`, `sexo`, `direccion`,
        `nombre`, `apellidopaterno`, `apellidomaterno`, `nacionalidad`, `idCarreras`, `usuario`) 
       ...
       // $result = $conn->query($sql); //$result = true/false on success or error respectively.
        ...
        if ($result) {
            sendResponse(200, $result , 'Estudiante Registration Successful.');
        } else {
            sendResponse(404, [] ,'Estudiante not Registered');
        }
        //close connection
        $conn->close();
    }
